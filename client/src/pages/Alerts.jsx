...
const Alerts = () => {
  ...
  const filtered = alerts.filter((a) => {
    ...
    const matchIP = searchIP === "" || a.ip.toLowerCase().includes(searchIP.toLowerCase());
  ...
  return (
    <div className="px-8 py-10">
      ...
      <div className="flex flex-wrap items-start gap-8 mb-8 bg-gray-900 border border-gray-800 rounded-xl p-6">
        <div>
          <p className="text-gray-400 text-xs uppercase tracking-wider mb-3">Filter by Severity</p>
            ...
            {severities.map((s) => (
              <button
                ...
                className={`px-4 py-1.5 rounded-full text-sm font-semibold transition ${
                  severityFilter === s ? "bg-purple-600 text-white" : "bg-gray-800 text-gray-400 hover:bg-gray-700"
                }`}
              >
                {s}
              </button>
        ...
        <div>
          <p className="text-gray-400 text-xs uppercase tracking-wider mb-3">Filter by Type</p>
          <div className="flex flex-wrap gap-2">
            {types.map((t) => (
              <button
                key={t}
      ...
      <div className="mb-8 bg-gray-900 border border-gray-800 rounded-xl p-6">
        <p className="text-gray-400 text-xs uppercase tracking-wider mb-3">Search by IP Address</p>
        <input
          type="text"
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {filtered.map((alert, index) => (
                <tr key={alert._id} className="border-b border-gray-800/50 hover:bg-gray-800/30 transition">
                  ...
                  <td className="py-3 text-gray-400 text-xs">{alert.status}</td>
                  <td className="py-3">
                    <button
      ...
      {selectedAlert && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
          <div className="bg-gray-900 border border-gray-700 rounded-xl p-6 w-[500px]">
            <h2 className="text-2xl font-bold mb-6">Alert Details</h2>
            <div className="space-y-4">
              <div>
                <p className="text-gray-400 text-sm">Alert ID</p>
                <p className="text-white font-mono text-xs">{selectedAlert._id}</p>
              </div>
              <div>
              ...
              <div>
                ...
                <span className={`px-2 py-1 rounded-full text-xs font-semibold ${severityColors[selectedAlert.severity]}`}>
